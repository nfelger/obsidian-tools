...
2. **In the project note** (one `vault.process` per project):
   - Find the matching task inside the Todo section
     ...
     `[[Project]]` prefix stripped — the same matching used by
     `dropTaskToProject`. If found and not yet completed, mark it `[x]`.
   - Append a log entry under the log heading
     (`logExtractionTargetHeading`), extract-log style: inserted directly
     after the heading (reverse-chronological), created at the end of the
     file when missing. The entry is a `### [[<source note>]]` sub-heading
     (one level below the log heading) followed by a **copy** of the
     completed task line (`[x]`, project-link prefix stripped, dedented)
     and its children. Multiple tasks completed for the same project in one
     invocation share a single sub-heading.
3. **In the source note**: mark each task `[x]` and leave it and its children
   in place. In a daily note the existing auto-move extension then carries it
   to `## Log` as usual.
...
- **Project copy not found, or already `[x]`:** still write the log entry and
  ...
  trail; the `[<]` flip is best-effort. Aborting would punish reworded tasks.
- **Copy, not move:** the daily note keeps the completed task and its
  children — the day's record stays intact and auto-move files it under
  `## Log`. Redundancy between daily log and project log is explicitly
  acceptable (WORKFLOW.md, "Text over Logic").
...
- Children copied verbatim (all children, including completed ones — this is
  a log entry, not a transfer, so `selectTransferableChildLines` does not
...
- Integration tests (markdown-first, `tests/integration/`):
  ...
  - project write failure → source untouched
